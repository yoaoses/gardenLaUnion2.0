interface FooterProps {
  nombre: string;
  corporacion: string;
  redes: {
    facebook?: string;
    instagram?: string;
    youtube?: string;
  };
}

const footerLinks = [
  { label: "Quiénes Somos", href: "#quienes-somos" },
  { label: "Sellos Educativos", href: "#sellos" },
  { label: "Niveles", href: "#niveles" },
  { label: "Noticias", href: "#noticias" },
  { label: "Admisión", href: "#admision" },
  { label: "Contacto", href: "#contacto" },
];

export default function Footer({ nombre, corporacion, redes }: FooterProps) {
  const year = new Date().getFullYear();

  return (
    <footer className="bg-gc-navy text-white/70">
      {/* Contenido principal */}
      <div className="container-gc py-12 lg:py-16">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          {/* Logo + info */}
          <div className="sm:col-span-2 lg:col-span-1">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-full bg-gc-gold/20 flex items-center justify-center font-display font-bold text-gc-gold">
                GC
              </div>
              <span className="font-display font-bold text-white text-lg">
                {nombre}
              </span>
            </div>
            <p className="text-sm font-body leading-relaxed mb-4">
              {corporacion}
            </p>
            <p className="text-sm font-body text-white/40">
              La Unión, Provincia del Ranco
              <br />
              Región de Los Ríos, Chile
            </p>
          </div>

          {/* Links rápidos */}
          <div>
            <h4 className="font-display font-semibold text-white mb-4 text-sm uppercase tracking-wider">
              Navegación
            </h4>
            <ul className="space-y-2">
              {footerLinks.map((link) => (
                <li key={link.href}>
                  <a
                    href={link.href}
                    className="text-sm font-body hover:text-gc-gold-light transition-colors"
                  >
                    {link.label}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* Documentos */}
          <div>
            <h4 className="font-display font-semibold text-white mb-4 text-sm uppercase tracking-wider">
              Documentos
            </h4>
            <ul className="space-y-2">
              <li>
                <a href="#" className="text-sm font-body hover:text-gc-gold-light transition-colors">
                  Proyecto Educativo (PEI)
                </a>
              </li>
              <li>
                <a href="#" className="text-sm font-body hover:text-gc-gold-light transition-colors">
                  Reglamento de Convivencia
                </a>
              </li>
              <li>
                <a href="#" className="text-sm font-body hover:text-gc-gold-light transition-colors">
                  Reglamento de Evaluación
                </a>
              </li>
              <li>
                <a href="#" className="text-sm font-body hover:text-gc-gold-light transition-colors">
                  Protocolos
                </a>
              </li>
            </ul>
          </div>

          {/* Redes sociales */}
          <div>
            <h4 className="font-display font-semibold text-white mb-4 text-sm uppercase tracking-wider">
              Síguenos
            </h4>
            <div className="flex gap-3">
              {redes.facebook && (
                <a
                  href={redes.facebook}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center hover:bg-gc-gold/20 hover:border-gc-gold/30 transition-colors"
                  aria-label="Facebook"
                >
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                  </svg>
                </a>
              )}
              {redes.instagram && (
                <a
                  href={redes.instagram}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center hover:bg-gc-gold/20 hover:border-gc-gold/30 transition-colors"
                  aria-label="Instagram"
                >
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                  </svg>
                </a>
              )}
              {redes.youtube && (
                <a
                  href={redes.youtube}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center hover:bg-gc-gold/20 hover:border-gc-gold/30 transition-colors"
                  aria-label="YouTube"
                >
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                  </svg>
                </a>
              )}
            </div>
          </div>
        </div>
      </div>

      {/* Barra inferior */}
      <div className="border-t border-white/10">
        <div className="container-gc py-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          <p className="text-xs font-body text-white/30">
            © {year} {nombre}. Todos los derechos reservados.
          </p>
          <p className="text-xs font-body text-white/20">
            La Unión, Región de Los Ríos, Chile
          </p>
        </div>
      </div>
    </footer>
  );
}
